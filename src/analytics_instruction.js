...
// Исследование ниши в других регионах — для поиска свежих идей контента
// Используется в обоих случаях: с Metricool и без
function buildNicheResearchPrompt(session) {
  const biz     = (session.businessProfile || '').slice(0, 600);
  const aud     = (session.audience || '').slice(0, 400);
  const region  = session.regionLabel || 'Латвия / Прибалтика';
  const lang    = session.contentLanguage || 'ru';
  const nicheKey = detectNiche(session.businessProfile);

  return `
Ты — ведущий маркетолог с глобальным опытом. Твоя задача: найти свежие идеи для контента на основе того что работает в этой нише в других странах и регионах.
...
ЗАДАЧА:
Исследуй что сейчас хорошо работает в нише "${nicheKey}" в других регионах — США, Великобритания, Германия, Скандинавия, Австралия (рынки где Instagram-маркетинг развит). Найди идеи которые ещё не стали мейнстримом в регионе клиента — это даст ему преимущество первопроходца.

Дай конкретные инсайты по каждому пункту:

ФОРМАТЫ КОТОРЫЕ НАБИРАЮТ СИЛУ В НИШЕ (в других регионах):
— [формат]: [почему работает, пример применения для этого бизнеса]
(3-4 пункта)

ТЕМЫ И УГЛЫ ПОДАЧИ КОТОРЫЕ ЦЕПЛЯЮТ АУДИТОРИЮ:
— [тема/угол]: [почему резонирует, как адаптировать для этого бизнеса]
(3-4 пункта)

ТРЕНДЫ В НИШЕ КОТОРЫЕ ЕЩЁ НЕ ДОШЛИ ДО РЕГИОНА КЛИЕНТА:
— [тренд]: [откуда пришёл, как использовать сейчас]
(2-3 пункта)

КОНКРЕТНЫЕ ИДЕИ ДЛЯ WAVE 2:
— [готовая идея поста/карусели/видео адаптированная под этот бизнес]
(3-4 идеи — конкретно, не абстрактно)

Пиши кратко и по делу. Только то что реально применимо для этого бизнеса.
`;
}

module.exports = { buildAnalyticsPrompt, buildContentCorrectionPrompt, buildNicheResearchPrompt, ANALYTICS_BENCHMARKS, ANALYTICS_ONBOARDING_TEXT };
