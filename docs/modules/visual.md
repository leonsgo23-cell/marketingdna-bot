...
- **Story Reel (29.06.2026)** → 7 изображений 9:16 генерируются параллельно с остальными; когда ≥2 готово — `generateFreeStoryReelVideo` собирает Creatomate видео (7 × 2.5с ≈ 17с) и отправляет в Bot3; НЕ блокирует кнопку "Отправить клиенту"
- **Кнопка "Отправить клиенту"** → появляется когда карусель, обложка И сторис уведомлены (Story Reel — независимо)
...
**Бэклог UI**: кнопки позиции текста / шрифта / музыки после получения видео в Bot3 (pending fix #16)
...
Каждый визуал сохраняется в двух версиях через `applyAndSaveOverlays`:

| Файл | Содержимое | Для чего |
|------|-----------|----------|
| `{chatId}_{section}_{i}_raw.jpg` | Чистое AI-фото без текста | Видео — Creatomate накладывает текст как отдельный слой |
| `{chatId}_{section}_{i}_ov.jpg` | То же фото + текст на языке клиента | Статичные посты (карусели, сторис, фото, обложки) |

**Секции**: `carouselSlides`, `photos`, `stories`, `covers`  
**Важно**: `_raw.jpg` не перезаписывается при повторном запуске без `/reset_client` — старый файл сохраняется. Для обновления raw — сделать сброс.

### Endpoint /reapply_overlays

Повторно накладывает текст на существующие изображения без перегенерации. Вызывается из Bot1 командой `/reapply_overlays {chatId}`.

Использует `extractAllCarouselTexts` для правильного извлечения текстов из всех 4 каруселей (оба формата: "Слайд N:" и "КАДР N: / Текст поверх фото:").  
Stories выводятся с позицией `'bottom'`.
