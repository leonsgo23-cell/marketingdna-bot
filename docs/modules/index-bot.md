...
| `block6_articles.js` | 105 | 3 SEO+GEO статьи. Использует: businessProfile, audience, regionLabel, headlines, castdevPhrases, semanticCore, competitorBrief, realNichePhrases. Контекст месяца: brandVoice (тон статьи), monthlyGoal (тематическая связь), monthlyFocus (актуальный контекст), clientStories (обезличенные примеры) |
| `block7_scripts.js` | ~1350 | Сценарии: карусели, фото, Stories, видео, **Story Reels** (29.06.2026). **Полный контекст промптов (29.06.2026)**: `clientContext` (brandVoice/monthlyGoal/monthlyFocus/clientStories/priceRange) + `painUtpBlock` (Q2: боль + УТП) + `customerJourneyBlock` (Q4: путь клиента → холодная/тёплая/горячая аудитория) + `objectionsBlock` (Q5: закрывать в хуке/слайдах/CTA) + `decisionMakerBlock` (Q8: CTA адресован лицу, принимающему решение) + `contentHistoryBlock` (Q6: усиливать что уже работало) + `castdevPhrases` + `realNichePhrases` + `reviewSitePhrases` + `semBlock` (семантика) + `competitorBlock` (конкуренты) + `visionStyle` + `existingStyle` + `rawContextBlock` (fallback на paidAnswers для платных клиентов). 4 функции генерации: `runBlock7`, `generateVideoScriptsFromSnap`, `generateStoryReelScripts`, `generateAllScriptsFromSnap` — все используют одинаковый набор блоков. `generateStoryReelScripts(snap, count)` — генерирует N рилсов (Старт=4, Стандарт=8, Профи=10) × 7 слайдов. Формат: `РИЛС N:` / `СЛАЙД N:` / `Текст: [3-5 слов]` / `Промпт для изображения:`. |
| `block8_covers.js` | 136 | ТЗ на обложки видео + обложки Highlights. Стандарт: 2 обложки + 4 Highlights. Профи: 4 обложки + 8 Highlights. Использует полный контекст: biz, aud, castdev, castdevPhrases, realNichePhrases, clientContext (brandVoice/monthlyGoal/monthlyFocus). Промпт с ШАГ 1-5 (язык аудитории → контекст месяца → тема видео → написать фразу). Сохраняет `session.covers` и `session.highlightCovers` |
| `block9_calendar.js` | 200 | Контент-план 15 дней (Wave1 или Wave2 через isWave2 флаг) |
| `block_free_package.js` | ~330 | Бесплатный пакет (7 дней). Генерирует параллельно: contentPlan, seoArticle, videoScript, carouselScript, coverExample, photoExample, storyExample, **storyReelScript** (29.06.2026). Формат карусели (29.06.2026): новый `КАДР N / Текст поверх фото / Промпт для изображения` (текст отдельно от изображения → корректное наложение overlay). storyReelScript: 7 слайдов × `СЛАЙД N / Текст / Промпт для изображения`, 9:16, нарратив хук→проблема→решение→CTA. |
...
## deliverVisualPackage — Wave1 / Wave2 (обновлено 17.06.2026)
...
- **Highlights** (🔵 Обложки Highlights): доставляются клиенту **только при Wave1** и только для Профи (8 шт) и Стандарт (4 шт). Источник: `results.highlights[]`.
...
4. Сохраняет `wave2_snapshot.json` с новыми Wave2 скриптами
5. **Перезаписывает `visual.json` Wave2-скриптами** (иначе `/generate` читает старый Wave1-файл)
6. Вызывает `/generate` → visual.js генерирует Wave2 визуалы → Bot3 на проверку → менеджер одобряет → клиент получает Wave2

> ⚠️ Шаг 5 критичен: без перезаписи `visual.json` Wave2 генерировала бы те же картинки что и Wave1 (баг исправлен 20.06.2026)
