...
| `block7_scripts.js` | ~1350 | Сценарии: карусели, фото, Stories, видео, **Story Reels** (29.06.2026). Все промпты используют полный контекст: biz, aud, castdev, castdevPhrases, realNichePhrases, reviewSitePhrases, clientContext (brandVoice/monthlyGoal/monthlyFocus/clientStories/priceRange), rawContext, visionStyle, existingStyle, analyticsBlock, historyBlock, legalRules, ctaInstruction. Новое (29.06): `generateStoryReelScripts(snap, count)` — генерирует N рилсов (Старт=4, Стандарт=8, Профи=10) × 7 слайдов. Формат: `РИЛС N:` / `СЛАЙД N:` / `Текст: [3-5 слов]` / `Промпт для изображения:`. Вызывается в `runBlock7` и `generateAllScriptsFromSnap`. Возврат `generateAllScriptsFromSnap` включает `storyReelScripts`. module.exports включает `generateStoryReelScripts`. |
